  ...
  async ({ packageName, version, description, author, license, private: isPrivate, type, directory }) => {
    try {
      ...
      if (packageExists) {
        ...
        if (isPrivate !== undefined) packageJson.private = isPrivate;
...
server.tool(
  ...
  async ({ packages, dev, peer, exact, global: isGlobal, directory }) => {
    try {
      ...
      if (packages && packages.length > 0) command += ` ${packages.join(' ')}`;
  ...
  async ({ script, args = [], directory }) => {
    try {
      ...
      if (!packageJson.scripts || !packageJson.scripts[script]) {
        throw new Error(`Script '${script}' not found in package.json. Available scripts are: ${
          packageJson.scripts ? Object.keys(packageJson.scripts).join(', ') : 'none'
        }`);
      }
...
server.tool(
  "npm-registry",
  "Get or set npm registry configuration",
  {
...
server.tool(
  ...
  async ({ fix, level, directory }) => {
    try {
      ...
      let command = 'audit';
      if (fix) command += ' fix';
      if (level) command += ` --audit-level=${level}`;
  ...
  async ({ query, limit = 10 }) => {
    try {
      ...
      try {
        ...
        results.forEach((pkg: any, index: number) => {

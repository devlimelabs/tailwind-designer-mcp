 ...
 * 
 * This is the command-line  from "@modelcontextprotocol/sdk/server/stdio.js";
...
program
  ...
  .option('-p, --port ', 'Port for SSE server (default: 3000)', '3000')
  .parse(process.argv);
...
async function main() {
  try {
    ...
    let transport;
    if (options.sse) {
      console.error(`Warning transport is not yet implemented. Using stdio transport instead.`);
      transport = new StdioServerTransport();
      // Future implementation:
      // transport = new SseServerTransport({ port(options.port, 10) });
    } else {
      transport = new StdioServerTransport();
    }
    ...
    console.error(`Domain Radar MCP Server is running on ${options.sse ? `SSE (port ${options.port})` } transport`);
